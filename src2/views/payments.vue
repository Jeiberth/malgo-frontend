        ...
        <div class="modal-content" style="max-width: 900px;">
          ...
          <div class="modal-body p-4" style="overflow-y: auto;">
              ...
              <div class="col-md-6">
                <div class="seccion" style="height: 100%; padding-bottom: 0px;">
                  ...
                  <div class="contenedor-campos" style="gap: 10px !important;">
                    <div v-for="(bill, index) in selectedPayment.payment_has_bills" :key="index" class="grupo-campos">
                      <div style="overflow-x: auto;">
                        <table class="table table-striped custom-border-table" style="border-radius: 25px; overflow: hidden;">
                          <tbody>
                            <tr>
                              <th style="width: 40%;">Nombre</th>
                              <td>{{ bill.bills.bill_types.name }}</td>
                            </tr>
                            <tr>
                              <th>Inmueble</th>
                              <td>{{ bill.bills.property.sku }}</td>
                            </tr>
                            <tr>
                              <th>Desde</th>
                              <td>{{ formatDate(bill.bills.start_date) }}</td>
                            </tr>
                            <tr>
                              <th>Hasta</th>
                              <td>{{ formatDate(bill.bills.end_date) }}</td>
                            </tr>
                            <tr>
                              <th>Vence</th>
                              <td>{{ formatDate(bill.bills.due_date) }}</td>
                            </tr>
                            <tr>
                              <th>Monto</th>
                              <td>{{ formatCurrency(bill.bills.amount) }}</td>
                            </tr>
                          </tbody>
                        </table>
                      </div>
                ...
                <div class="seccion" style="height: 100%;">
                  ...
                  <div class="contenedor-campos" style="height: calc(100% - 60px);">
                    <div class="grupo-campos" style="height: 100%;">
                      <img :src="selectedPayment.payment_cert" alt="Comprobante de pago" style="width: 100%; height: 100%; object-fit: contain; border: 0px solid #d7b377; border-radius: 15px;">
                    </div>
                  </div>
                </div>
              </div>
            ...
            <div class="modal-footer">
              ...
              <button v-if="selectedPayment.status !== 'verified'" type="button" class="boton-primario" @click="verifyPayment" :disabled="verifying" style="width: auto;">
          ...
          <div class="row">
            <div class="col-md-6 mb-3">
              <select class="entrada" v-model="propertyFilter" style="background-color: #f8f5f0;">
                ...
                <option v-for="sku in uniquePropertySkus" :key="sku" :value="sku">{{ sku }}</option>
              </select>
            </div>
            <div class="col-md-6 mb-3">
            ...
            <table class="table table-bordered table-hover table-striped align-middle text-center custom-big-table" style="border-radius: 25px; overflow: hidden;">
              ...
              <tbody>
                <tr v-for="payment in paginatedPayments" :key="payment.id">
                  <td>
                    <!-- <div v-for="(bill, index) in payment.payment_has_bills" :key="index">
                      {{ bill.bills.property.sku }} {{ bill.bills.bill_types.name }}
                    </div> -->
                    <div v-for="(bill, index) in payment.payment_has_bills" :key="index">
                        {{ bill.bills.property.sku }}
                        <i :class="getBillIcon(bill.bills.bill_types.name)"></i><br>
                    </div>
                  </td>
...
<script setup>
  ...
  const generateFakePayments = () => {
    ...
    for (let i = 1; i <= 25; i++) {
      ...
      payments.push({
        id: i,
        amount: paymentBills.reduce((sum, bill) => sum + parseFloat(bill.bills.amount), 0).toFixed(2),
        payment_method: paymentMethods[Math.floor(Math.random() * paymentMethods.length)],
        status: statusOptions[Math.floor(Math.random() * 2)],
        payment_cert: 'https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcTr_Gc86Sq0aRtKyv2gCjyvIpKWsuJHbb9_TA&s',
  ...
  const filteredPayments = computed(() => {
    return payments.value.filter(payment => {
      const matchesProperty = !propertyFilter.value ||
        payment.payment_has_bills.some(bill => bill.bills.property.sku === propertyFilter.value);
      const matchesStatus = !statusFilter.value ||
        (statusFilter.value === 'verified' ? payment.status === 'verified' : payment.status === 'pending');
  ...
  const uniquePropertySkus = computed(() => {
    const skus = new Set();
    payments.value.forEach(payment => {
      payment.payment_has_bills.forEach(bill => {
        skus.add(bill.bills.property.sku);
      });
    });
    ...
    const verifyPayment = async () => {
        ...
        await billPaymentResource.verifyPayment({id: selectedPayment.value.id}).then(response => {
            if (response && response.success) {
                ...
                Swal.fire(
                    'Éxito',
                    'El pago fue verificado exitosamente.',
                    'success'
                );
            } else {
                Swal.fire(
