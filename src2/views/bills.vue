    ...
    <div class="container">
        ...
        <div v-if="showPaymentModal" class="modal" @click.self="closePaymentModal">
            <div class="modal-content" style="overflow-y: auto; height: auto; max-width: 880px;">
                <div class="modal-header">
                    <span class="modal-title" >💳 Pagar Factura(s)</span>
                                ...
                                <div class="grupo-campos">
                                    <div class="campos-grid">
                                        ...
                                        <div class="campo">
                                            ...
                                            <input type="file" @change="handlePaymentProofUpload" accept=".pdf,.jpg,.jpeg,.png" class="entrada" :class="{ 'is-invalid': errors.payment_proof }">
        ...
        <div class="row seccion mainSeccion mt-4">
            <div class="col-12 d-flex justify-content-between align-items-center mb-4">
                ...
                >
                    <i class="fas fa-money-bill-wave" style="margin-right: 10px;"></i>Pagar Factura
                </button>
            </div>
<!-- 
                    ...
                    <table class="table table-bordered table-striped table-hover align-middle text-center custom-big-table" style="border-radius: 25px; overflow: hidden;">
                        ...
                        <tbody>
                            <tr v-for="bill in bills" :key="bill.id">
                                <!-- <td>
                                    <input type="checkbox" v-model="selectedBills" :value="bill.id" :disabled="bill.status !== 'pending'" >
                                </td> -->
                                <td>
                                    <input type="checkbox" v-model="selectedBills" :value="bill.id" style="height: 100%;">
                                </td>
                                <td>{{ bill.bill_types.name }}</td>
...
<script setup>
    ...
    const handlePaymentProofUpload = (event) => {
        ...
        if (file && (file.type === 'application/pdf' || file.type.startsWith('image/'))) {
            ...
            errors.value.payment_proof = 'Solo se permiten archivos PDF o imágenes';
        }
    };
    ...
    const validatePayment = () => {
        ...
        if (!paymentData.value.payment_proof) {
            errors.value.payment_proof = 'Comprobante de pago requerido';
            isValid = false;
        }
    ...
    const submitPayment = async () => {
        ...
        await billPaymentResource.payBills().then(response => {
            if (response.success) {
                bills.value = bills.value.filter(bill => !selectedBills.value.includes(bill.id));
                Swal.fire(
                    '¡Pago exitoso!',
                    'Las facturas han sido marcadas como pagadas.',
                    'success'
                );
            } else {
                Swal.fire(
