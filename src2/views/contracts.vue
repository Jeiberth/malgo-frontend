                                    ...
                                    <div class="campos-grid">
                                        ...
                                        <div class="campo">
                                            ...
                                            <select v-model="newContract.tenant_application_id" class="entrada" :class="{ 'is-invalid': errors.tenant_application_id }">
                                                ...
                                                {{ app.property_id }} - {{ app.FinancialResponsible[0].full_name }}
                                                </option>
                                            </select>
                ...
                <div style="overflow-x: auto;">
                    ...
                    <tbody>
                        <tr v-for="contract in paginatedContracts" :key="contract.id">
                            ...
                            <td>
                                <a v-if="contract.contract_doc" @click.prevent="openDocModal(contract.contract_doc)">
                                <i class="fas fa-file-pdf" style="color: black;"></i>
                                </a>
                            </td>
                            <td>{{ contract.created_at }}</td>
                            <td><i class="fa-solid fa-trash" @click="deleteContract(contract.id)"></i></td>
                        </tr>
                    </tbody>
                    </table>
                </div>
...
<script setup>
    ...
    const contractResource = new contractApi();
    ...
    const properties = ref([
        { id: 1, sku: 'FO-APTO-301' },
        { id: 2, sku: 'SE-APTO-201' },
        { id: 3, sku: 'SE-APTO-301' },
        { id: 4, sku: 'FO-PARQ-1' }
    ]);

    const tenantApplications = ref(Array.from({ length: 10 }, (_, i) => ({
        id: i + 1,
        property_id: properties.value[Math.floor(Math.random() * 4)].sku,
        FinancialResponsible: [{ full_name: ['Laura', 'Carlos', 'María', 'Andrés'][Math.floor(Math.random() * 4)] }]
    })));
    ...
    //         application => application.status !== 'Pendiente' && application.status !== 'Pendiente Rechazado'
    //     )
    ...
    const uniquePropertySkus = computed(() => [...new Set(contracts.value.map(c => c.property.sku))]);
    const statusOptions = ['En curso', 'Terminado'];
    ...
    async function deleteContract(id){
        ...
        if (resultado.isConfirmed) {
            ...
            await contractResource.deleteContract({"id": id}).then(response => {
                if( response.success ){
    ...
    const submitContract = async () => {
        ...
        await contractResource.createContract({"id": id}).then(response => {
            if( response.success ){
